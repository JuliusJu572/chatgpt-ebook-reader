...
const EbookParser = (() => {
  ...
  async function parseTXT(file) {
    ...
    const normalized = normalizeTxtParagraphs(text);                                                             
    const segments = createSegmentsFromText(normalized, { spineIndex: 0, href: file.name });  
  ...
  function normalizeTxtParagraphs(text) {
    // 分割所有换行（兼容 Windows \r\n / Unix \n）
    ...
    for (const line of lines) {
        ...
        // 当前行为空行
        if (trimmed === '') {
            // 缓冲区有内容，说明一段结束，存入结果并清空缓冲
            if (buffer) {
                result.push(buffer);
                buffer = '';
            }
        } else {
            // 当前行有文字，拼接至缓冲区（空格分隔多行同段内容）
